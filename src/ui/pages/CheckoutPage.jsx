  ...
  return (
    <main className={wide ? undefined : "fincla-scroll"} style={{ ...G, height: "100%", minHeight: 0, display: "flex", flexDirection: "column", overflowX: "hidden", overflowY: wide ? "hidden" : "auto", boxSizing: "border-box", background: "#F7F8F5", color: T.ink, padding: wide ? "0 20px" : "0 16px 32px" }}>
      <div style={{ width: "100%", maxWidth: 1120, minHeight: 0, margin: "0 auto", display: "flex", flexDirection: "column", flex: wide ? 1 : undefined }}>
        ...
        {quote && <div style={{ display: "grid", gridTemplateColumns: wide ? "minmax(0, 1.22fr) minmax(310px, .78fr)" : "1fr", gap: wide ? 20 : 14, alignItems: "start", marginTop: wide ? (compactViewport ? 10 : 16) : 0, flex: wide ? 1 : undefined, minHeight: 0, overflowY: wide ? "hidden" : "visible" }}>
          <Card style={{ height: wide && ((activeSection && activeSection !== "plan") || constrainedViewport) ? "100%" : "auto", minHeight: 0, display: wide ? "flex" : "block", flexDirection: "column", overflow: "hidden", border: `1px solid ${T.border}`, order: wide ? 0 : 1 }}>
            <CheckoutPanel mobile={!wide} scrollable={wide && compactViewport} number="1" title="Plano e ciclo" detail="Escolha como prefere pagar" summary={`${planName} · ${cycleName} · ${money(quote.total_cents)}`} open={activeSection === "plan"} complete={accountVisited || Boolean(accountDetails) || session?.isAuthenticated} onOpen={() => toggleSection("plan")}>
            ...
            <CheckoutPanel mobile={!wide} scrollable={wide} number="3" title="Pagamento" detail="Finalize sua assinatura" summary={`Cartão de crédito · ${money(quote.total_cents)}`} open={activeSection === "payment"} locked={!session?.isAuthenticated && !accountDetails} onOpen={() => toggleSection("payment")}>
...
function CheckoutPanel({ number, title, detail, summary, open, complete, locked, keepMounted = false, onOpen, children, scrollable = false, mobile = false }) {
  ...
  return <section aria-labelledby={`checkout-step-${number}`} style={{ borderBottom: number === "3" ? 0 : `1px solid ${T.border}`, padding: open ? (mobile ? "18px 16px 0" : "22px 24px 0") : (mobile ? "15px 16px" : "18px 24px"), flex: scrollable && open ? "1 1 auto" : "0 0 auto", minHeight: 0, display: scrollable ? "flex" : "block", flexDirection: "column" }}>
    <button type="button" disabled={locked} onClick={onOpen} style={{ position: "relative", flexShrink: 0, display: "flex", flexWrap: mobile ? "wrap" : "nowrap", alignItems: "center", gap: mobile ? "3px 11px" : 11, width: "100%", padding: 0, border: 0, background: "transparent", color: locked ? "#AEB5BE" : T.ink, textAlign: "left", cursor: locked ? "not-allowed" : "pointer" }}>
      ...
      <div style={{ minWidth: 0, paddingRight: mobile ? 24 : 0 }}><h2 id={`checkout-step-${number}`} style={{ fontSize: 16, margin: 0, color: locked ? "#9DA5AF" : undefined }}>{title}</h2><p style={{ color: locked ? "#B5BCC4" : T.inkGhost, fontSize: 12, margin: "3px 0 0" }}>{locked ? "Disponível após a etapa anterior" : detail}</p></div>{summary && !open && <span title={summary} style={{ order: mobile ? 3 : undefined, width: mobile ? "calc(100% - 38px)" : undefined, marginLeft: mobile ? 38 : "auto", marginTop: mobile ? 4 : undefined, maxWidth: mobile ? undefined : "45%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: locked ? "#B5BCC4" : T.inkMid, fontSize: 12, fontWeight: 700 }}>{summary}</span>}<span aria-hidden="true" style={{ position: mobile ? "absolute" : undefined, right: mobile ? 0 : undefined, top: mobile ? 6 : undefined, marginLeft: mobile ? undefined : (summary && !open ? 12 : "auto"), color: locked ? "#B5BCC4" : T.inkMid, fontSize: 16, lineHeight: 1 }}>{open ? "▴" : "▾"}</span>
    </button>
